(ns huntharvest.phase
  "Phase machine: the states a wildlife-harvest record transits through.

  State machine:
    :intake -> :survey -> :advise -> :treat -> :record -> :audit

  `:intake` is harvest-request receiving (licensed hunter/trapper,
  species, requested harvest method); `:survey` is wildlife-population/
  habitat-condition assessment; `:advise` is the advisor's harvest/
  trapping-scheduling recommendation; `:treat` is the actual licensed
  hunting/trapping field operation performed; `:record` is logging the
  completed harvest (evidence, safety/compliance parameters) into
...
(def all-phases
  "All valid phases in the wildlife-harvest workflow."
  [:intake :survey :advise :treat :record :audit])

(def phase-sequence
  "Ordered phases representing normal harvest-record progression."
  [:intake :survey :advise :treat :record :audit])
